...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!PauseMenu.isPaused && !DeathScreen.isPaused) {
            ...
            if (enableJump && Input.GetKeyDown(jumpKey) && isGrounded)
            {
                ...
                // Play jump sound and stop walking or sprinting audio
                // SoundManager.Instance.PlaySound(SoundManager.Instance.jump);
            }

            // Check if the player is in the air after a jump
            if (!isGrounded && isJumping)
            {
                // Player is in the air, stop walking and sprinting sounds
                SoundManager.Instance.walking.Stop();
                SoundManager.Instance.sprinting.Stop();
            }
            else
            {
                // Reset the isJumping flag when the player is grounded
                isJumping = false;
            }
            ...
            {
                // Player is moving
                if (isGrounded && !isJumping) // Check if the player is grounded and not jumping
                {
                    // Play walking sound if the game is not paused
                    if (!SoundManager.Instance.walking.isPlaying)
                    {
                        SoundManager.Instance.PlaySound(SoundManager.Instance.walking);
                    }

                    // Check if the player is sprinting
                    if (Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
                    {
                        // Player is sprinting, play sprinting sound if the game is not paused
                        if (!SoundManager.Instance.sprinting.isPlaying)
                        {
                            SoundManager.Instance.PlaySound(SoundManager.Instance.sprinting);
                        }
                    }
                    else
                    {
                        // Player is not sprinting, stop sprinting sound
                        SoundManager.Instance.sprinting.Stop();
                    }
                }
                else
                {
                    // Player is in the air or jumping, stop walking and sprinting sounds
                    SoundManager.Instance.walking.Stop();
                    SoundManager.Instance.sprinting.Stop();
                }
            }
            else
            {
                ...
                // Player is not moving, stop sprinting sound
                SoundManager.Instance.sprinting.Stop();
            }
        }
        else
        {
            // Game is paused, stop all movement sounds
            SoundManager.Instance.walking.Stop();
            SoundManager.Instance.sprinting.Stop();
        }

        // Disables cursor lock when we are in the pause screen
        HandleCursorLock();
        // Disables crosshair while we are in the pause screen
        UpdateCrosshair();
    }
    ...
    {
        if (!PauseMenu.isPaused && !EventSystem.current.IsPointerOverGameObject() && !DeathScreen.isPaused)
        {
            bool isInventoryOpen = InventorySystem.Instance.isOpen;
            bool isCraftingOpen = CraftingSystem.Instance.isOpen;

            // Check if both the inventory and crafting UIs are closed
            if (!isInventoryOpen && !isCraftingOpen)
            {
                // If both UIs are closed, lock the cursor
                Cursor.lockState = CursorLockMode.Locked;
            ...
            {
                // If either inventory or crafting UI is open, unlock the cursor
